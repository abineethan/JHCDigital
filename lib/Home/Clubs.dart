...
class ClubsCacheManager extends CacheManager {
  ...
  factory ClubsCacheManager() => _instance;
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          final clubsQuery = firestore.collection('Clubs');

          final activitiesStream =
              clubsQuery.snapshots().switchMap((clubsSnapshot) {
            if (clubsSnapshot.docs.isEmpty) {
              return Stream.value(<QueryDocumentSnapshot>[]);
            }

            final activityStreams = clubsSnapshot.docs.map((clubDoc) {
